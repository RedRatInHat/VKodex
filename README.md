...
- **Codex Desktop:** запуск приложения должен получать `CODEX_CLI_PATH` с этим путём. В `CODEX_SOURCES` это можно задать через `launcher.type: "command"`, `launcher.environment` и `scripts/start-codex-desktop.ps1`. Скрипт каждый раз разрешает текущую AppX-версию, поэтому обновление Desktop не оставляет launcher на удалённом versioned-пути. Запуск ссылки через системный обработчик не гарантирует передачу переменной.
- Пакет Desktop-адаптера, подготовленный из `%LOCALAPPDATA%\OpenAI\Codex\bin\<version>\codex.exe`, сохраняет стабильный корень `bin` и при каждом старте выбирает актуальный `codex.exe`. Обновление приложения больше не требует пересобирать пакет только из-за смены versioned-каталога.
...
После атомарного переключения VK-привязки выполняется архивация источника. Результат проверяется по записи исходной задачи и сохранённой границе истории, а не по исчезновению из списка. Адаптер владельца передаёт команду через клиент, который держит задачу; живой тест этого пути пройден в VS Code `.codex-work`. Для Codex Desktop путь подготовлен, но на рабочих задачах ещё не проверен. Если адаптера нет, запись через отдельный App Server допускается только после подтверждения, что задача не загружена в клиенте и её последний ход завершён. Неизвестный результат записи никогда не повторяется вслепую: VKodex проверяет архив чтением. Если после копирования изменились история или цель источника, архивация останавливается и обе копии сохраняются для сверки. VKodex не закрывает Codex и не снимает блокировки принудительно. До живого теста Desktop и переноса туда-обратно полную автономность для всех конфигураций считать подтверждённой нельзя.
