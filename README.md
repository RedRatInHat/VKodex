...
| `/new`, «Новая задача» | Создать пользовательскую задачу: каталог Codex → проект или «Без проекта» → локальная папка или отдельный Git worktree → название → стартовый промпт → модель → уровень рассуждения. |
