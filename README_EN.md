...
A regular message continues the linked task. During an active turn it becomes a steer; after completion it starts the next turn in the same context. Any participant in the linked conversation may author a prompt: the bridge ignores only its own community messages and already processed outbound messages. VKodex prepends transport metadata with the author's display name and VK ID, allowing Codex to distinguish participants in a shared conversation; profile names are cached, and the ID remains available if the VK API lookup fails. The block is explicitly marked as attribution rather than an instruction. The bridge does not inspect the participant list. A request is not retried automatically when Codex state is uncertain or a response was lost.
