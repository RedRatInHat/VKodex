...
Фоновое наблюдение также разделено по задачам: медленное чтение истории, проверка создаваемой задачи или обновление панелей не удерживает общий цикл подключений. Незавершённое задание не запускается повторно на каждом тике. Health отдельно показывает этап и возраст задержки (`history`, `catalog`, `creator-inspect`, `panels`), даже когда сам таймер моста исправен. Общий каталог по-прежнему нужен для фонового поиска ещё не подключённых задач; это ожидание не блокирует обслуживание существующих подписок.

Внутри одного соединения App Server повтор идентичного незавершённого серверного запроса не вызывает обработчик вопроса или инструмента заново. Разные данные с тем же ожидающим RPC ID отклоняются без закрытия остальных задач; поздний ответ обработчика не попадает в новое соединение. Защита касается ожидающих запросов, а не долговременного восстановления завершённых операций.

Отписка от наблюдения не удаляет ожидающий вопрос Codex: задача может продолжать работать без подписчика. Вопрос закрывается при ответе, замене, подтверждённой выгрузке задачи или потере её соединения. Перед отправкой ответа повторно проверяется, что вопрос не сменился за время подготовки; ошибка одного наблюдателя вопросов не мешает остальным.
...
Входящие IPC-broadcast принимаются как с адресатом этого клиента, так и без списка адресатов: именно так native-клиент объявляет начало наблюдения при открытии задачи. Чужие, пустые и некорректные списки адресатов игнорируются; обработчики состояния дополнительно проверяют владельца, каталог и задачу.

Наблюдатели одной native-задачи внутри одного IPC-транспорта также используют общую подписку. Отписка отправляется только после закрытия последнего наблюдателя; каждый получает отдельную копию состояния. Конфликтующие копии одной задачи из разных каталогов не объединяются.
