...
| `/health`, «Проверить здоровье» | Немедленно перепроверить VK, очередь, SQLite, каталоги Codex, named pipe, stream protocol и активные трансляции. |
...
- Во время работы хода внизу переписки циклически меняется `думаю...` → `думаю..` → `думаю.`. Если последнее сообщение — комментарий агента, индикатор добавляется в его конец. После вашего сообщения, меню или другого ответа бот при необходимости отправляет новый тихий индикатор ниже; старый больше не анимируется. Обновление — раз в шесть секунд. После завершения строка исчезает из комментария, отдельный индикатор показывает `Готово.`; ошибки и потеря связи показываются явно. При отключении трансляции редактирование прекращается.
...
- размер и возраст активной очереди VK, а также паузу rate limit;
...
Это готовая проверка для планировщика, NSSM, systemd или другого внешнего наблюдателя. При двух последовательных состояниях `DEGRADED`/`FAILED` бот отправляет одно уведомление в менеджер. При смене причины приходит новый сигнал; после восстановления — отдельное сообщение `health check снова OK`. Если сам VK недоступен, предупреждение остаётся в устойчивой очереди и отправляется после восстановления связи.

`DEGRADED` означает, что основная работа может продолжаться, но часть цепочки не подтверждена: например, нет открытой задачи для protocol canary, выполняющаяся задача потеряла live-подключение или очередь не очищается более 30 секунд. Закрытые и бездействующие задачи могут не держать live-подписку и сами по себе не ухудшают health-state: при новом запросе они подключаются через доступный desktop/SDK-путь. `FAILED` означает отказ обязательной проверки или очередь старше пяти минут. После обновления Codex дополнительно выполните `desktop:probe`, `/health` и тестовый ход в отдельной задаче. Не меняйте вручную версию IPC, чтобы обойти отказ адаптера.
...
Каталоги Codex не редактируются вручную. Для переименования, архива, назначения проекта и экспорта адаптер использует ограниченный набор методов `codex app-server --stdio`. Создание новой задачи и резервное продолжение незагруженной задачи выполняет официальный Codex SDK; live-события уже открытой задачи идут через IPC десктопа. Это разные интерфейсы, и мост не считает один молчаливой заменой другого при несовместимой версии протокола.
