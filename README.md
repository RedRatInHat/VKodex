...
**Статус: экспериментальное подключение к десктопу Windows.** Для live-событий VKodex использует внутренний IPC приложения, который может измениться после обновления Codex; совместимость автоматически проверяется при запуске и во время работы. App Server атомарно материализует новую задачу её первым ходом в выбранном профиле, после чего настроенный launcher открывает её в Codex Desktop или VS Code; все последующие запросы передаются подтверждённому live-владельцу. Начните с проверки на неважной задаче.
...
| `CODEX_SOURCES` | Нет | Предпочтительный JSON-массив источников с `home` и launcher. Если задан, заменяет `CODEX_HOME` и `CODEX_EXTRA_HOMES`. |
...
### Экспериментальный адаптер владельца задачи

Если источник переноса удерживается нативным клиентом, отдельный App Server не всегда может его архивировать. Адаптер `VKodexOwnerLauncher.exe` передаёт запрос архивации через тот же App Server, которым пользуется клиент. Это не дополнительный исполнитель ходов: обычные запросы, уведомления и подтверждения остаются в исходном соединении.
...
- **Codex Desktop:** запуск приложения должен получать `CODEX_CLI_PATH` с этим путём. В `CODEX_SOURCES` это можно задать через `launcher.type: "command"` и `launcher.environment`. Запуск ссылки через системный обработчик не гарантирует передачу переменной.
...
Статус: архивация через владельца проверена на отдельном VS Code. Полная интеграция в оба рабочих клиента и автономный перенос туда-обратно ещё не подтверждены. Адаптер проверяет источник и неархивированных потомков любой глубины: все должны иметь подтверждённое состояние простоя и неактивную цель. Непроверенные, выгруженные или работающие потомки блокируют архивацию; автоматически загружать или останавливать их адаптер не пытается. Неопределённый результат архивации не повторяется автоматически. Каталог `owner-transports` содержит локальные токены доступа: не публикуйте его и не отправляйте в VK.
...
| `/open`, «Поделиться» → «Открыть в Codex» | Явно вывести настроенное приложение Codex на передний план и открыть эту задачу. При обычном промпте открытие происходит только для восстановления отсутствующего live-подключения. |
...
Для исходящей папки сохраняются отдельные пределы: до 10 файлов, до 20 МиБ на файл и до 50 МиБ суммарно. Мост отправляет только обычные файлы из `outbox/`: скрытые файлы пропускаются, ссылки и файлы, которые ещё записываются, отклоняются. Пути из текста ответов и остальные каталоги проекта не сканируются. Архивы автоматически не распаковываются. Содержимое документов остаётся пользовательскими данными, а не командами для моста.
...
По умолчанию VKodex ищет задачи в `~/.codex` и открывает их через зарегистрированный протокол Codex Desktop. Для нескольких аккаунтов используйте `CODEX_SOURCES`: каждый каталог связывается с приложением, авторизованным в этом профиле.
...
- `desktop` открывает `codex://threads/<id>` через зарегистрированный Codex Desktop;
...
Для live-чтения существующей задачи и всех ходов после создания мост использует владельца задачи в настроенном клиенте и проверяет, что путь истории относится к выбранному каталогу. Первое открытие при привязке записывается в базу: сам перезапуск VKodex не открывает все связанные задачи. При новом промпте мост сначала проверяет live-подключение. Если Codex явно подтверждает отсутствие владельца задачи, например после перезапуска приложения, VKodex один раз вызывает launcher выбранного каталога, ждёт подключения до 30 секунд, проверяет источник истории и передаёт этот промпт. Такое восстановление может показать окно Codex; уже подключённая задача повторно не открывается. Фоновое чтение и проверки состояния не вызывают launcher. При несовпадении пути истории, неправильном аккаунте, общей ошибке IPC или несовместимой версии live-протокола команда блокируется; резервный SDK-ход не запускается. Отправленная команда с неизвестным результатом автоматически не повторяется.
...
Перед копированием фиксируется последний завершённый ход и состояние исходного журнала. Создаётся новый thread ID в выбранном `CODEX_HOME`; модель, уровень рассуждения и рабочая папка сохраняются. Paginated-история адаптируется через временную копию JSONL в `<целевой CODEX_HOME>/.vkodex-transfer-staging`. До переключения VK-беседы проверяются граница истории, неизменность источника, нативные название и назначение проекта, а также готовность живого клиента назначения — в том числе для варианта **Без проекта**. Проект, который каталог лишь вывел из пути рабочей папки, не считается подтверждённым назначением. После открытия клиента название и проект проверяются заново.
...
После атомарного переключения VK-привязки выполняется архивация источника. Результат проверяется по точной записи исходной задачи в каталоге и сохранённому журналу, а не по её исчезновению из списка активных задач. При временном сбое VK-беседа остаётся на проверенной копии; архивация повторяется в фоне. **Повторить архивацию** запускает проверку того же этапа. Если исходный клиент удерживает журнал для записи (`already has an active writer`), запросы записи останавливаются сразу. У установленного Desktop/VS Code клиента нет команды архивации во внешнем follower IPC: источник нужно архивировать в его клиенте. После этого VKodex сам заметит результат при проверке раз в минуту и завершит сохранённый этап. VKodex не закрывает Codex и не снимает блокировки принудительно. **Полностью автономная архивация открытого источника пока не поддерживается**; это ограничение остаётся видимым в меню и health-check, перенос не выдаётся за завершённый.

`/health` отдельно показывает незавершённые переносы, операции без прогресса и операции, требующие решения пользователя. Старые записи переноса без сохранённой границы истории не запускаются автоматически и не блокируют сообщения в существующих беседах. Их можно проверить или отменить через меню; недостающие сведения не восстанавливаются догадкой. Если источник уже архивирован и точная VK-привязка подтверждена, старая запись закрывается с отдельной пометкой: это сверка исторической операции, а не повторная проверка всей её истории.

Проверки восстановления выполняются на отдельных тестовых задачах: аварийное завершение процесса после сохранения ID fork и после переключения привязки, потеря ответа после открытия клиента, перенос цели на паузе с оставшимся бюджетом и обработка большого JSONL. Ожидание завершения нативного процесса отделено от закрытия его потоков вывода: задержка Windows при закрытии stdio не должна превращать подтверждённый fork в ошибку.
...
Обработчик каждой live-задачи дополнительно проверяется в фоне каждые 30 секунд. Если после перезапуска приложения он сменился, мост переподключает подписку без открытия окна и без повторного запуска промпта. Если обработчик отсутствует, индикатор «думаю» останавливается. `/menu` доступно и без связи; кнопка «Открыть в Codex» или `/open` явно открывает задачу через launcher выбранного каталога. Ошибка `systemError` не считается работающим ходом, даже если в восстановленной истории осталось старое `inProgress`. При исчерпании лимита бот предлагает `/limits`, а не продолжает анимацию.
...
| Задача видна, но подключение не удалось | Codex и VKodex должны работать под тем же пользователем ОС. Состояние `notLoaded` означает, что задача выгружена из памяти, а не удалена. Проверьте launcher выбранного источника в `CODEX_SOURCES`: он должен открыть приложение с тем же `CODEX_HOME` и аккаунтом. VKodex ждёт live-владельца до 30 секунд и не запускает отдельный SDK-ход. Для дубликатов ID также проверьте каталог источника и путь истории. |
...
Каталоги Codex не редактируются вручную. Для целей, переименования, архива, назначения проекта, экспорта и чтения аккаунтных лимитов адаптер использует ограниченный набор методов `codex app-server --stdio`. Состояние цели выбирается по `CODEX_HOME` конкретной задачи, поэтому одинаковые ID из разных каталогов не смешиваются. Создание — отдельная ограниченная граница: App Server материализует задачу и выполняет только первый ход, потому что live follower-протокол не принимает временный `client-new-thread` ID. События этого хода поступают в тот же поток VK, `/stop` отменяет его, а launcher сразу открывает выбранный клиент. После завершения сессия создания уничтожается; продолжение, остановка и события существующей задачи идут только через live IPC. При несовместимой версии протокола мост блокирует изменение вместо запуска запасного исполнителя.
