...
| `/limits`, «Лимиты Codex» | Показать использованный процент аккаунтных лимитов, время сброса, тариф и доступные кредиты. Команда также работает в связанной беседе и не передаётся агенту. |
...
Каталоги Codex не редактируются вручную. Для переименования, архива, назначения проекта, экспорта и чтения аккаунтных лимитов адаптер использует ограниченный набор методов `codex app-server --stdio`. Создание новой задачи и резервное продолжение незагруженной задачи выполняет официальный Codex SDK; live-события уже открытой задачи идут через IPC десктопа. Это разные интерфейсы, и мост не считает один молчаливой заменой другого при несовместимой версии протокола.
