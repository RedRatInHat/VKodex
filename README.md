...
Supervisor сохраняет вывод каждого запуска локально в `BOT_DATA_DIR/logs`: обычный вывод — в `vkodex-<запуск>.stdout.log`, ошибки и стеки аварий — в `vkodex-<запуск>.stderr.log`, историю запусков и кодов завершения — в `supervisor.log`. Отдельные файлы не перезаписываются при следующем запуске. Эта папка входит в приватный `data/`, исключённый из Git. Не публикуйте журналы без просмотра: сообщения сторонних библиотек могут содержать локальные пути и технические данные.
