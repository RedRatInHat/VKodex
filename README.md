...
В служебных ответах менеджера есть кнопка **Меню**. В меню видны процесс и время работы моста, количество задач и связей, состояние подключений, очередь отправки и результат автоматической проверки live-протокола. Полное меню открывается только по запросу; отдельное приветствие при запуске не присылается.
...
После запуска и затем каждые десять минут VKodex проверяет доступность named pipe. Если среди последних задач есть открытая в десктопе, canary также подтверждает stream protocol v11; результат виден в меню менеджера. После обновления Codex дополнительно выполните `desktop:probe` и проверьте тестовую задачу. Не меняйте вручную версию IPC, чтобы обойти отказ адаптера.
